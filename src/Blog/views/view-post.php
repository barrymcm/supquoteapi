...
$post = $entityManager->getRepository('Blog\Entity\Post')->findPostWithComments($_GET['id']);
...
<head>
	<meta charset="utf-8">
	<title>
		<?=htmlspecialchars($post->getTitle())?> - My blog
	</title>
</head>
<body>
	<article>
		...
		<h2>Comments</h2>
			...
			<?php foreach ($post->getComments() as $comment): ?>
				...
				<article>
					...
					<p>
						<?=htmlspecialchars($comment->getBody())?>
					</p>
					<a href="delete-comment.php?id=<?=$comment->getId()?>">
						Delete this comment
					</a>
				</article>
