...
{
	// Create a new post if a post has not been retrieved and 
	// set its date of publication
	if (!isset ($post)) 
	{
		$post = new Post();
	...
	<form method="POST">
		<label>
			...
			<input type="text" name="title" value="
				<?= isset ($post) ? htmlspecialchars($post->getTitle()) : ''?>"
			maxlength="255" required>
		</label>
		<br>
		<label>
			Body
			<textarea name="body" cols="20" rows="10" required>
		...
		<label>
			...
			<input type="text" name="tags" value="
				<?=isset($post) ? htmlspecialchars(
					implode(', ', $post->getTags()->toArray())) : ''?>" required>
		</label><br>
		<input type="submit">
	</form>
